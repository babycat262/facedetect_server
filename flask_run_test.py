...
#ff_pb_path = "./pb_model/asia_face_graph.pb"
...
def face_landmark():
    ...
    for i in range(len(output_dict['detection_scores'])):
        if output_dict['detection_scores'][i] > 0.1:
            ...
            pred = pred[0]
            # cv2.imwrite("503_landmark.jpg", face_data)
            res = []
            ## 裁剪之后的人脸框中的坐标
...
def upload():
    ...
    # upload_path = os.path.join("tmp/tmp-" + str(time.time()) + "." + f.filename.split(".")[-1])
    # secure_filename(f.filename))  #注意：没有的文件夹一定要先创建，不然会提示没有该路径
    print(upload_path)
    f.save(upload_path)
    return upload_path
...
@app.route('/face_dis')
def face_dis():
    im_data_1 = read_image("./test_image/503/0000.png")
    im_data_1 = np.expand_dims(im_data_1, axis=0)
    emb1 = face_feature_sess.run(ff_embeddings,
                                 feed_dict={ff_images_placeholder: im_data_1,
                                            ff_train_placeholder: False})
    ##相同人脸
    im_data_1 = read_image("./test_image/503/0000.png")
    ##不同人脸
    # im_data_1 = read_image("./test_image/500/0001.png")
    # im_data_1 = read_image("./test_image/503/0000.png")
...
if __name__ == "__main__":
    app.run(host='172.18.134.4', debug=True)
